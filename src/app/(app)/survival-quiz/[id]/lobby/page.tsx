...
import type { SurvivalGameRoom, SurvivalPlayer, User as FsUser } from '@/lib/types';
...
export default function SurvivalLobbyPage() {
    ...
    const [kickCandidate, setKickCandidate] = useState<SurvivalPlayer | null>(null);
    ...
    const joinRoom = useCallback(async (userToJoin: FsUser, roomRef: any) => {
        ...
        try {
            await runTransaction(db, async (transaction) => {
                ...
                const roomData = roomDoc.data() as SurvivalGameRoom;

                if (roomData.players && roomData.players[userToJoin.uid]) return; // Already in room

                if (Object.keys(roomData.players).length >= 100) {
                    toast({ variant: 'destructive', title: '방이 꽉 찼습니다.', description: '최대 100명까지 참여 가능합니다.' });
    ...
    const handleLeaveRoom = async () => {
        ...
        if (isHost) {
            await deleteDoc(doc(db, 'survival-game-rooms', gameRoom.id));
        } else {
            await updateDoc(doc(db, 'survival-game-rooms', gameRoom.id), {
    ...
    if (isLoading || loadingUser) {
        return (
          <div className="flex flex-col items-center justify-center min-h-screen">
            ...
            <p className="mt-4 text-muted-foreground">서바이벌 로비에 참여하는 중...</p>
          </div>
        );
    }
        ...
        <div className="container mx-auto py-8">
            <Card className="max-w-4xl mx-auto">
                <CardHeader className="text-center">
                    ...
                    <CardDescription>모든 플레이어가 들어오면 호스트가 게임을 시작합니다. 최대 100명까지 참여 가능합니다.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-8">
                    ...
                    <div className="space-y-4">
                        <h3 className="font-headline text-xl font-semibold flex items-center gap-2">
                            ...
                            <span>참여한 플레이어 ({players.length} / 100)</span>
                        </h3>
